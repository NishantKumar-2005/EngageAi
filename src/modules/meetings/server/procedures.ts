...
import { createTRPCRouter, premiumProcedure, protectedProcedure } from "src/trpc/init";
...
export const meetingsRouter = createTRPCRouter({
    ...
    getTranscript: protectedProcedure.input(z.object({id : z.string()})).query(async ({ input, ctx }) => {
        ...
        const speakerIds = [
            ...new Set (response.map((item) => item.speaker_id)),
          ];
          ...
          const transcriptWithSpeakers = response.map((item) => {
    ...
    generateToken: protectedProcedure.mutation(async ({ ctx }) => {
        ...
        const token = streamVideo.generateUserToken({
          ...
          validity_in_seconds: Math.floor(Date.now() / 1000) - 60,
        })
